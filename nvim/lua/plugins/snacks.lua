...
map('<leader>lj', ':%!jq .<cr>', 'Pretty json')
map('<leader>tn', '<cmd>tabnew<cr>', 'new Tab')
map('<leader>nt', '<cmd>term<cr>', 'new Terminal')
map('<leader>lw', ':set wrap linebreak breakindent showbreak=<cr>', 'Toggle Wrap')
...
map('<M-h>', '<cmd>vertical resize -2<CR>', 'Decrease window width')
map('<M-l>', '<cmd>vertical resize +2<CR>', 'Increase window width')
map('<M-j>', '<cmd>resize +2<CR>', 'Increase window height')
map('<M-k>', '<cmd>resize -2<CR>', 'Decrease window height')
